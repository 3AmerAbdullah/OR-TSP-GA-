...
# parameters
numbers_of_generations = 10 # in other words, iteration size
...
def draw_path(solution):
    ...
    AA= len(solution.chromosome)

    print('\n\n\n')
    print("Path order (Node coordinates):")
    for i in range(0,AA-2):
       print(f" {solution.chromosome[i].id}",end=' --->')

    print(f" {solution.chromosome[AA-2].id}",'\n\n\n')
